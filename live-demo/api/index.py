...
app = FastAPI(title="AI Commerce Ops Live Demo", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class RunRequest(BaseModel):
    preset: Literal["exam-table-paper", "patient-monitor", "clinic-trolley"]
...
def _enrich_with_langchain(product: dict) -> Enrichment:
    ...
    model = ChatOpenAI(model=_env("OPENAI_MODEL") or "gpt-5.6-luna", max_retries=2)
    ...
    try:
        ...
        with psycopg.connect(database_url, connect_timeout=5) as conn:
            with conn.cursor() as cur:
                ...
                """)
                decision = {"enrichment": enrichment.model_dump(), "approval_required": approval_required}
                cur.execute("""
                    ...
                    ON CONFLICT (request_id) DO UPDATE SET
                        ...
                        updated_at = NOW()
                """, (request_id, product["sku"], json.dumps(product), json.dumps(decision)))
            conn.commit()
        return True
    except Exception:
        return False
...
def _shopify_token() -> tuple[str, int]:
    ...
    if not client_id or not client_secret:
        raise HTTPException(status_code=503, detail="Shopify client credentials are not configured")
    shop = _shop_domain()
    ...
    if response.status_code >= 400:
        raise HTTPException(status_code=502, detail=f"Shopify auth failed ({response.status_code})")
    body = response.json()
    token = body.get("access_token")
    if not token:
        raise HTTPException(status_code=502, detail="Shopify auth response did not include access_token")
...
""".strip()
    variables = {
        "input": {
            ...
            "status": "DRAFT",
            "tags": ["ai-commerce-ops", "portfolio-demo", "langchain", "shopify-api", product["sku"]],
            "seo": {"title": enrichment.seo_title, "description": enrichment.seo_description},
        },
    ...
    if response.status_code >= 400:
        raise HTTPException(status_code=502, detail=f"Shopify GraphQL HTTP error ({response.status_code})")
    payload = response.json()
    if payload.get("errors"):
        raise HTTPException(status_code=502, detail={"shopify_graphql_errors": payload["errors"]})
    result = payload.get("data", {}).get("productSet", {})
    if result.get("userErrors"):
        raise HTTPException(status_code=502, detail={"shopify_user_errors": result["userErrors"]})
    shopify_product = result.get("product")
    if not shopify_product:
        raise HTTPException(status_code=502, detail="Shopify productSet returned no product")
    return {**shopify_product, "token_expires_in": expires_in, "mode": "DRAFT_UPSERT"}

def _run_direct(preset: str) -> RunResponse:
    request_id = str(uuid.uuid4())
    product = {**PRESETS[preset], "preset": preset}
    trace: list[TraceItem] = []
    t0 = time.perf_counter()
    trace.append(TraceItem(step="Product intake", status="success", duration_ms=max(1, int((time.perf_counter() - t0) * 1000)), detail=f"Validated {product['sku']}"))
    t0 = time.perf_counter()
    enrichment = _enrich_with_langchain(product)
    trace.append(TraceItem(step="LangChain enrichment", status="success", duration_ms=int((time.perf_counter() - t0) * 1000), detail=f"GPT-5.6 Luna → {enrichment.category}"))
    t0 = time.perf_counter()
    approval_required = _approval_required(product, enrichment)
    trace.append(TraceItem(step="Risk gate", status="review" if approval_required else "success", duration_ms=max(1, int((time.perf_counter() - t0) * 1000)), detail=f"Risk {enrichment.risk_score}/100 · " + ("human review required" if approval_required else "auto-draft allowed")))
    t0 = time.perf_counter()
    audit_logged = _audit(request_id, product, enrichment, approval_required)
    trace.append(TraceItem(step="PostgreSQL audit", status="success" if audit_logged else "skipped", duration_ms=max(1, int((time.perf_counter() - t0) * 1000)), detail="Audit row persisted" if audit_logged else "DATABASE_URL not configured; audit skipped"))
    shopify = None
    if approval_required:
        trace.append(TraceItem(step="Shopify draft", status="skipped", duration_ms=0, detail="Blocked by human-in-the-loop gate"))
        workflow_status = "pending_human_approval"
    else:
        t0 = time.perf_counter()
        shopify = _shopify_upsert(product, enrichment)
        trace.append(TraceItem(step="Shopify Admin API", status="success", duration_ms=int((time.perf_counter() - t0) * 1000), detail=f"DRAFT synced · {shopify['id']}"))
        workflow_status = "shopify_draft_synced"
    return RunResponse(request_id=request_id, orchestrator="direct-api-fallback", preset=preset, product=product, enrichment=enrichment, approval_required=approval_required, workflow_status=workflow_status, shopify=shopify, audit_logged=audit_logged, trace=trace)

@app.get("/api/health")
def health() -> dict:
    return {
        "status": "ok",
        "openai_model": _env("OPENAI_MODEL") or "gpt-5.6-luna",
        "shopify_configured": bool(_env("SHOPIFY_CLIENT_ID") and _env("SHOPIFY_CLIENT_SECRET") and _env("SHOPIFY_SHOP")),
...
def run_demo(request: RunRequest) -> RunResponse:
    ...
    if n8n_url:
        ...
        try:
            ...
            raise HTTPException(status_code=502, detail=f"n8n orchestration failed: {type(exc).__name__}") from exc
